...
class DbImport():
  ...
  # creare le sottoclassi opportune in funzione del tipo di archivio
  ...
  def connectPg(self):
      # si connette al database Postgres locale. Ridefinire in caso di database remoto   
        conn = self.psycopg2.connect(database="e20", user="giulio", password="benoni58",host = "127.0.0.1")
        return conn 
...
class AziendePgSQLite(DbImport):
    ...
    queryLettura = "SELECT categoria, nome, descrizione, indirizzo, citta, cap, provincia, phone, mail, web, facebook, twitter, num_immagini, servizi, altri_servizi, googlemap, orari FROM ristoranti;"
